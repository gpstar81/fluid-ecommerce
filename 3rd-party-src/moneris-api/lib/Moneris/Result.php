...
class Moneris_Result
{
	...
	protected $_messages = array(
		...
		self::ERROR_DECLINED => 'Credit card has been declined.',
		self::ERROR_NOT_AUTHORIZED => 'Not authorized',

		self::ERROR_CVD => 'Invalid 3 digit credit card CV security code provided. Please check your credit card information and try again.',
		self::ERROR_CVD_NO_MATCH => 'The provided 3 digit credit card security code did not match.',
		self::ERROR_CVD_NOT_PROCESSED => 'Credit card 3 digit security code not processed.',
		self::ERROR_CVD_MISSING => 'Credit card 3 digit security code not provided.',
		self::ERROR_CVD_NOT_SUPPORTED => 'Credit card 3 digit security code not supported by this credit card.',

		self::ERROR_AVS => 'Billing address verification failed. Please check your credit card billing address under the payment settings and try again.',
		self::ERROR_AVS_POSTAL_CODE => 'Billing address verification failed. Incorrect postal code supplied. Please check your credit card billing address under the payment settings and try again.',
		self::ERROR_AVS_ADDRESS => 'Billing address verfication failed. Incorrect credit card billing address supplied. Please check your credit card billing address under the payment settings and try again.',
		self::ERROR_AVS_NO_MATCH => 'Billing address verification failed. The provided credit card billing address did not match. Please check your credit card billing address under the payment settings and try again.',
		self::ERROR_AVS_TIMEOUT => 'Billing address verification timed out.',
	...
	public function avs_response() {
		$receipt = $this->transaction()->response()->receipt;
		$gateway = $this->transaction()->gateway();

		if ($gateway->check_avs()) {
			if ($gateway->check_avs() && isset($receipt->AvsResultCode)	&& 'null' !== (string) $receipt->AvsResultCode && ! in_array($receipt->AvsResultCode, $gateway->successful_avs_codes())) {
